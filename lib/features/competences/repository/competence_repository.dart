...
abstract class CompetenceRepository {
  Future<List<RankingModel>> fetchGlobalRanking(int languageId, int timeFilterId);
}
...
class SupabaseCompetenceRepository implements CompetenceRepository {
  ...
  @override
  Future<List<RankingModel>> fetchGlobalRanking(int languageId, int timeFilterId) async {
    try {
      final response = await _supabase.rpc(
        'get_global_ranking',
        params: {
          'p_language_id': languageId,
          'p_time_filter_id': timeFilterId,
        },
      );
      final rankingList = (response as List).map((json) => RankingModel.fromJson(json)).toList();
      return rankingList;
    } catch (e) {
...
class MockCompetenceRepository implements CompetenceRepository {
  ...
  Future<List<RankingModel>> fetchGlobalRanking(int languageId, int difficultyId) async {
    ...
    for (int i = 1; i <= 30; i++) {
      ...
      if (i == 1) { // Usuario Top 1 de prueba
        userId = otherUserUUID;
        username = 'Usuario1';
        profileName = 'Competidor 1';
      } else if (i == 2) { // Tu usuario de prueba
        userId = currentUserUUID;
        username = 'dxniel7';
        profileName = 'Dxniel7';
      } else { // El resto de usuarios falsos
        userId = 'fake-id-$i';
        username = 'Usuario$i';

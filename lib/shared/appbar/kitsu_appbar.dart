...
class _KitsuAppBarState extends ConsumerState<KitsuAppBar> {
      ...
      return Container(
          ...
          child: Row(
            ...
            children: [
              ...
              _buildStatItem(icon: Icons.local_fire_department, color: Colors.grey, text: "..."),
              _buildStatItem(icon: Icons.emoji_events, color: Colors.grey, text: "..."),
              _buildStatItem(icon: Icons.favorite, color: Colors.grey, text: "..."),
            ],
          ));
    }
  ...
  Widget _buildLanguageSelector(BuildContext context, WidgetRef ref, AppBarState stats) {
  ...
  Widget _buildLanguageMenu(BuildContext context, WidgetRef ref, int currentLangId) {
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData) {
          children = snapshot.data!.map((lang) {
            ...
            return _buildLanguageMenuItem(langName, langAsset, langId, textTheme, colorScheme);
          }).toList();

        } else if (snapshot.hasError) {
          children = [Text("Error", style: TextStyle(color: colorScheme.onError))];
        } else {
  ...
  Widget _buildLanguageMenuItem(String langName, String langAsset, int langId, TextTheme textTheme, ColorScheme colorScheme) {
    return InkWell(
      onTap: () {

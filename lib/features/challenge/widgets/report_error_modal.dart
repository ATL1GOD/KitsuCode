// lib/features/challenge/widgets/report_error_modal.dart
...
class _ReportErrorModalState extends ConsumerState<ReportErrorModal> {
  ...
  bool _loading = false;
  String? _errorMessage; // Para mostrar errores dentro del modal
  bool _showSuccess = false; // Para mostrar mensaje de éxito
  ...
  Future<void> _submit() async {
    ...
    /// VALIDACIÓN 1: Debe seleccionar una razón
    if (_selectedReason == null) {
      setState(() => _errorMessage = 'Por favor, selecciona un motivo para el reporte.');
      return; // NO cerramos el modal
    }

    /// VALIDACIÓN 2: Si seleccionó "Otro", debe escribir algo
    if (_selectedReason == ReportReason.other &&
        _otherController.text.trim().isEmpty) {
      setState(() => _errorMessage = 'Por favor, describe el problema en la sección "Otro".');
      return; // NO cerramos el modal
    }

    /// VALIDACIÓN 3: Debe haber un usuario autenticado
    final userId = ref.read(authStateProvider).value?.session?.user.id;

    if (userId == null) {
      setState(() => _errorMessage = 'No se pudo identificar tu usuario. Intenta cerrar sesión y volver a entrar.');
      return;
    }
    ...
    try {
      ...
      // Éxito: mostramos mensaje de éxito DENTRO del modal
      if (mounted) {
        setState(() => _showSuccess = true);
        
        // Esperamos 2 segundos para que vean el mensaje y luego cerramos
        await Future.delayed(const Duration(seconds: 2));
        if (mounted) Navigator.pop(context);
      }
    } catch (e) {
      // Error de red: mostramos error DENTRO del modal
      if (mounted) {
        setState(() {
          _loading = false;
          _errorMessage = 'Error de conexión. No se pudo enviar tu reporte. Intenta de nuevo.';
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: _showSuccess ? _buildSuccessView(colors, textTheme) : _buildFormView(colors, textTheme),
      ),
    )
        .animate()
                    ...
                    onTap: _loading
                        ...
                        : () {
                            setState(() {
                              ...
                              _errorMessage = null; // Limpiar error al seleccionar
                            });
                          },

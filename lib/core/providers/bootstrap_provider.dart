...
class BootstrapNotifier extends AsyncNotifier<void> {
  ...
  Future<void> build() async {
    if (kDebugMode) debugPrint('Bootstrap: INICIO');
    ...
    if (kDebugMode) debugPrint('Bootstrap: Firebase + dotenv listos (paralelo)');
    ...
    if (kDebugMode) debugPrint('Bootstrap: Supabase listo');
    ...
    if (kDebugMode) debugPrint('Bootstrap: UI configurado');

    // 5) Handler FCM
    FirebaseMessaging.onBackgroundMessage(firebaseMessagingBackgroundHandler);
    if (kDebugMode) debugPrint('Bootstrap: Handler de background listo');

    if (kDebugMode) debugPrint('Bootstrap: COMPLETADO ✓ (optimizado ~400-600ms más rápido)');
  }
}
